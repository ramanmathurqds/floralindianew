...
<?php if(isset($listingPageInfo->results->Name) && !isset($data->results->error)) { ?>
    ...
    <div data-simplebar data-simplebar-auto-hide="false" class="filter-box">
        <input type="hidden" value="<?=$ProductCategoryID?>" id="ProductCategoryID" />
        ...
        <input type="hidden" value="<?=urldecode($subMenuOfMenu)?>" id="subMenuOfMenu" />
        <input type="hidden" value="<?=urldecode($menuName)?>" id="menuName" />

        <div class="filter-header">
            <div class="patch"></div>
            <button class="btn-close"><img src="<?=DOMAIN?>Content/assets/images/common/close-light.png" alt="btn-close" /></button>
...
<?php } ?>
            ...
            <ul>
                ...
                <?php
                    if(isset($data->results)) {
                        $BreadCrumbName = isset($listingPageInfo->results->Name) ? $listingPageInfo->results->Name : $CatName;
                ?>
                    <li><a href="javascript:;"><?=$BreadCrumbName?></a></li>
                <?php } ?>
            </ul>
        </div>
    
    <!--if mobile don't render this-->
    <?php if(!isMobile()) { ?>
        <?php if(isset($listingPageInfo->results->Name)) { ?>
            <div class="category-promo">
                <div class="row">
                    <div class="col-5 custom-grid">
                        <a href="<?=$listingPageInfo->results->Name?>">
                            <img src="<?=DOMAIN?><?=$listingPageInfo->results->ProductListPageImageURL?>" alt="<?=$listingPageInfo->results->Name?>" loading="lazy" class="img-100" />
                        </a>
                    </div>

                    <div class="col-7 custom-grid embed-video shimmer">
                        <iframe src="<?=$listingPageInfo->results->ProductListPageVideoURL?>" frameborder="0" loading="lazy" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    </div>
                </div>
            </div>
        <?php } ?>
        ...
        <!--
            <div class="no-data">
                <img src="<?=DOMAIN?>Content/assets/images/common/no-data-found.png" alt="no-data-found"/>
            </div>
        -->
        ...
        <div class="keypoints">
            <div class="row">
                <div class="col custom-grid">
                    <a href="<?=DOMAIN?><?=$countryCode?>/all-listing/sameday-delivery">
                        <div class="keybox">
                            <div class="keyicon">
                                <img src="<?=DOMAIN?>Content/assets/images/common/sameday-delivery-icon.png" alt="Same day delivery" />
                            </div>
                ...
                <div class="col custom-grid">
                    <a href="<?=DOMAIN?><?=$countryCode?>/all-listing/midnight-delivery">
                        <div class="keybox">
                            <div class="keyicon">
                                <img src="<?=DOMAIN?>Content/assets/images/common/midnight-delivery-icon.png" alt="Midnight Delivery" />
                            </div>

                            <div class="keyinfo">
                                <h2>Be the first to wish!</h2>
                ...
                <div class="col custom-grid">
                    <a href="javascript:;">
                        <div class="keybox">
                            <div class="keyicon">
                                <img src="<?=DOMAIN?>Content/assets/images/common/sameday-delivery-icon.png" alt="custom delivery" />
                            </div>

                            <div class="keyinfo">
                                <h2>Custom Products</h2>
                                <p>Customise Yourself</p>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                    </a>
    ...
    <?php } ?>
        <?php
        if(isset($listingPageInfo->results->Name)) {
            if(isset($subCatOfCat->results)) {
        ?>
            <div class="gift-category">
                <div class="giftCategoryCarosuel owl-carousel owl-theme gift-option-wrapper">
                <?php
                    foreach ($subCatOfCat->results as $key => $value) {
                        if(isset($value->ProductSubCategoryName)) {
                ?>
                    <a class="btn-category redirect active slider-item" href="<?=DOMAIN?><?=$countryCode?>/listing/product/<?=strtolower(str_replace(" ", "-", $CatName))?>/<?=strtolower(str_replace(" ", "-", $value->ProductSubCategoryName))?>/<?=$ProductCategoryID?>" data-value="<?=$value->ProductSubCategoryName?>">
        ...
        <?php
            if(isset($data->results) && isset($data->results->Products)) {
        ?>
            <div class="product-list-block">
                ...
                <div class="product-head">
                    <div class="row">
                        ...
                        <?php if(isset($data->results) && isset($data->results->Products)) { 
                            
                            if(!empty($subMenuOfMenu)) {
                                $name = urldecode($subMenuOfMenu);
                            } else {
                                $name = $listingPageInfo->results->Name;
                            }
                ...
                <div class="product-listing-wrapper">
                    ...
                    foreach ($data->results->Products as $key => $value) {
                        if(isset($value->ProductName)) {
                            $oneDayDelivery = isset($value->isOneDayDelivery) === '1' ? ' oneDay' : '';
                            $price = ' '.$value->Price;
                            ...
                            <a href="<?=DOMAIN?><?=$countryCode?>/product/<?=strtolower(str_replace(" ","-",$value->ProductName))?>/<?=$ProductCategoryID?>/<?=$value->ProductID?>">
                                ...
                                <div class="product-info text-center">
                                    ...
                                    <div class="product-price">
                                        <p>
                                            <span class="webCurrency">
                                                <span class="setDefaultCurrency"><?=$CurrencyLogo?></span>
                                            </span> 
                                            <span class="setCurrBasedPrice"><?=$value->Price?></span> &nbsp;&nbsp;
                                            <span class="cancelled-price">
                                                <span class="webCurrency">
                                                    <span class="setDefaultCurrency"><?=$CurrencyLogo?></span>
                                                </span> 
                                                <span class="setCurrBasedPrice"><?=$value->Mrp?></span>
                                            </span>
                                        </p>
                                    </div>
                                </div>
            ...
            <?php 
                $output = '<br /><div align="center" class="c-main-pagination">
